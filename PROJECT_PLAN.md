...
### タスク2.1: 設定構造体の定義
- [ ] `internal/config/config.go` ファイル作成
- [ ] `Config` 構造体の定義
  - [ ] DuckDNS設定 (domain, token)
  - [ ] 更新間隔設定 (interval)
  - [ ] IP取得ソース設定 (ip_sources)
  - [ ] ログ設定 (level, format)
...
### タスク2.4: バリデーション機能
- [ ] 必須項目チェック (domain, token)
- [ ] interval のフォーマットチェック
- [ ] IP取得ソースのURL検証
- [ ] バリデーションエラーのわかりやすいメッセージ
...
- [ ] `internal/ip/fetcher.go` ファイル作成
- [ ] `Fetcher` インターフェース定義
- [ ] `Fetch(ctx context.Context) (string, error)` メソッド
...
- [ ] `internal/duckdns/client.go` ファイル作成
- [ ] `Client` 構造体定義
  - [ ] HTTPクライアント
  - [ ] リトライ設定
...
- [ ] API URL構築: `https://www.duckdns.org/update`
- [ ] クエリパラメータ設定 (domains, token, ip)
- [ ] HTTPリクエスト送信
...
- [ ] `internal/scheduler/scheduler.go` ファイル作成
- [ ] `Scheduler` 構造体定義
  - [ ] 更新間隔
  - [ ] IP Fetcher
  - [ ] DuckDNS Client
...
| フェーズ2: 設定管理 | 4 | 0 | 0% | 未着手 |
| フェーズ3: IP取得機能 | 4 | 0 | 0% | 未着手 |
| フェーズ4: DuckDNS API | 4 | 0 | 0% | 未着手 |
| フェーズ5: スケジューラー | 4 | 0 | 0% | 未着手 |
